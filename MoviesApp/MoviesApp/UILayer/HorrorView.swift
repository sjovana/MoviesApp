...
struct HorrorView: View {
    
    @ObservedObject var viewModel: ViewModel

    init(dependencies: HorrorDependencies){
            ...
            VStack{
                ...
                ScrollView {
                    LazyVGrid(columns: columns){
                        ForEach(viewModel.horrorMovies){movie in
...
protocol HorrorDependencies : HomeViewModelDependencies, DetailViewModelDependencies {

}
extension HorrorView {
    final class ViewModel : ObservableObject{
        
        let dependencies: HorrorDependencies
        
        @Published var viewState: ViewState = .initial
        @Published var nowPlayingg = [NowPlayingData]()
        @Published var movies: [MovieData] = []
        @Published var horrorMovies: [NowPlayingData] = []
        ...
        var filteredMovies : [NowPlayingData] {
            let horrorMovieIds = movies.filter{$0.genresName == "horror" } .map { $0.id}
            return nowPlayingg.filter { horrorMovieIds.contains($0.id) }
        }
        ...
        func loadNowPlayinggggggg() {
            ...
            dependencies.nowPlayingUseCase.fetchNowPlaying()
                .sink(receiveCompletion: { completion in
                    ...
                    self.filterHorrorMovies()
                    self.viewState = .finished
                })
                .store(in: &subscriptions)
        }

        private func filterHorrorMovies() {
            ...
            for movie in nowPlayingg {
                ...
                dependencies.movieUseCase.fetchMovie(id: movie.id)
                    .sink(receiveCompletion: { _ in
                        ...
                        if movieDetails.genresName.contains("Horror") {
                            DispatchQueue.main.async {
                                self.horrorMovies.append(movie)
                            }
                        }
                    })
