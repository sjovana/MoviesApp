...
struct ComedyView: View {
    ...
    init(dependencies: ComedyDependencies){
...
protocol ComedyDependencies : HomeViewModelDependencies, DetailViewModelDependencies {

}
extension ComedyView {
    final class ViewModel : ObservableObject{
        let dependencies: ComedyDependencies
        
        @Published var viewState: ViewState = .initial
        @Published var nowPlayingg = [NowPlayingData]()
        @Published var movies: [MovieData] = []
        @Published var comedyMovies: [NowPlayingData] = []
        ...
        var filteredMovies : [NowPlayingData] {
            let comedyMovieIds = movies.filter{$0.genresName == "comedy" } .map { $0.id}
            return nowPlayingg.filter { comedyMovieIds.contains($0.id) }
        }
        ...
        func loadNowPlayinggggggg() {
            ...
            dependencies.nowPlayingUseCase.fetchNowPlaying()
                .sink(receiveCompletion: { completion in
                    ...
                    self.nowPlayingg = data
                    self.filterComedyMovies()
                    self.viewState = .finished
                })
                .store(in: &subscriptions)
        }

        private func filterComedyMovies() {
            ...
            for movie in nowPlayingg {
                ...
                dependencies.movieUseCase.fetchMovie(id: movie.id)
                    .sink(receiveCompletion: { _ in
                        ...
                        if movieDetails.genresName.contains("Comedy") {
                            DispatchQueue.main.async {
                                self.comedyMovies.append(movie)
                            }
                        }
                    })
